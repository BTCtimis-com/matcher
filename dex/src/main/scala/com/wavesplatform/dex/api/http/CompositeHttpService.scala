...
class CompositeHttpService(apiTypes: Set[Class[_]], routes: Seq[ApiRoute], settings: RestAPISettings) extends ScorexLogging {
  ...
  private val notFound: Route =
    mkTracedRoute("/notFound") {
      complete(StatusCodes.NotFound)
    }

  val compositeRoute: Route = extendRoute(routes.map(_.route).reduce(_ ~ _)) ~ swaggerRoute ~ notFound
