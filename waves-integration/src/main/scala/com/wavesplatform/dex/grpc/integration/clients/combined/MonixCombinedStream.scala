...
class MonixCombinedStream(
  ...
  override val stream: Observable[WavesNodeEvent] = internalStream
  ...
  mergedEvents
    .foldLeft[Status](Status.Starting()) {
      case (orig, Left(evt)) =>
        utxEventsTransitions(orig, evt).tap { updated =>
          blockchainStatus = updated
          log.info(s"utx: $orig + $evt -> $updated")
        }
      case (orig, Right(evt)) =>
        blockchainEventsTransitions(orig, evt).tap { updated =>
          blockchainStatus = updated
  ...
  override def updateProcessedHeight(height: Int): Unit = processedHeight = height
  override def currentProcessedHeight: Int = processedHeight
